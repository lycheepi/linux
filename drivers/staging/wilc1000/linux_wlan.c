...
static int init_irq(struct net_device *dev)
{
	...
	struct wilc *wl;
        ...
        np = of_find_compatible_node(NULL, NULL, "atmel-wilc3000");
        if (!np) {
                printk("Failed to find node\n");
                return -1;
        }
	...
	wl = vif->wilc;
        ...
        wl->gpio = of_get_named_gpio(np, "gpio-irq", 0);
...
/* IWG27M: USDHC3: Workaround for SDIO WIFI Power On Sequence */
#ifdef CONFIG_IWG27M
		gpio_direction_output(GPIO_NUM_CHIP_EN, 1);
		mdelay(5);
		gpio_direction_output(GPIO_NUM_RESET, 1);
