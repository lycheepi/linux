...
#ifdef FSC_HAVE_SRC
static int ReadSourceCapabilities(FSC_U8 *data, FSC_S32 bufLen,
                                  struct Port *port)
{
    FSC_U32 i = 0, j = 0;
    FSC_U32 index = 0;
...
static int ReadReceivedCaps(FSC_U8 *data, FSC_U32 bufLen, struct Port *port)
{
    FSC_U32 i = 0, j = 0;
    FSC_U32 index = 0;
    FSC_U32 len = port->CapsHeaderReceived.NumDataObjects * 4;
    ...
    data[index++] = port->CapsHeaderReceived.byte[0];
    data[index++] = port->CapsHeaderReceived.byte[1];

    for (i = 0; i < port->CapsHeaderReceived.NumDataObjects; i++)
    {
        for (j = 0; j < 4; j++)
        {
            data[index++] = port->CapsReceived[i].byte[j];
        }
    }
...
static void WriteSourceCapabilities(FSC_U8 *data, struct Port *port, FSC_U32 len)
{
    FSC_U32 i = 0, j = 0;
...
static FSC_BOOL ProcessTCWrite(FSC_U8 *subCmds, struct Port *port)
{
    ...
    for (i = 0; i < HCMD_PAYLOAD_SIZE / 2; i++)
    {
        ...
        case TYPEC_DFP_CURRENT_AD:
            if (pCmd->val != port->SourceCurrent && pCmd->val < utccInvalid)
            {
                port->SourceCurrent = pCmd->val;
...
static FSC_BOOL ProcessDPWrite(FSC_U8 *subCmds, struct Port *port)
{
    ...
    for (i = 0; i < HCMD_PAYLOAD_SIZE / 2; i++)
    {
        ...
        {
        case DP_ENABLE:
            port->DpEnabled = (pCmd->val == 0) ? FALSE : TRUE;
            break;
        case DP_AUTO_MODE_ENTRY:
            port->DpAutoModeEntryEnabled = (pCmd->val == 0) ? FALSE : TRUE;
            break;
        case DP_MASK:
            if (len < sizeof(FSC_U32)) { goto end_of_cmd; }
            port->DpModeEntryMask.word = READ_WORD((&pCmd->val + 1));
            pCmd = (struct pd_buf*) ((FSC_U8*) pCmd + pCmd->val);
            break;
        case DP_STATUS:
            if (len < sizeof(FSC_U32)) { goto end_of_cmd; }
            port->DpStatus.word = READ_WORD((&pCmd->val + 1));
            pCmd = (struct pd_buf*) ((FSC_U8*) pCmd + pCmd->val);
            break;
        case DP_MODE_ENTRY_VALUE:
            if (len < sizeof(FSC_U32)) { goto end_of_cmd; }
            port->DpModeEntryValue.word = READ_WORD((&pCmd->val + 1));
            pCmd = (struct pd_buf*) ((FSC_U8*) pCmd + pCmd->val);
            break;
        case DP_EOP:
        default:
                              ...
                              struct Port *port)
{
    ...
    for (i = 0; i < HCMD_PAYLOAD_SIZE / 2; i++)
    {
        ...
        {
        case DP_ENABLE:
            pRsp->val = port->DpEnabled;
            break;
        case DP_AUTO_MODE_ENTRY:
            pRsp->val = port->DpAutoModeEntryEnabled;
            break;
        case DP_MASK:
            if (len < sizeof(FSC_U32)) break;
            pRsp->val = WRITE_INT((&pRsp->val + 1),
                    port->DpModeEntryMask.word);
            pRsp = (struct pd_buf*)((FSC_U8*)pRsp + pRsp->val);
            break;
        case DP_STATUS:
            if (len < sizeof(FSC_U32)) break;
...
static void ProcessDeviceInfo(HostCmd_t *outMsg)
{
    ...
    outMsg->deviceInfo.cmd.rsp.hostcom[0] = 1; /* Low */
