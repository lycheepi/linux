...
void StateMachineTypeC(Port_t *port)
{
    ...
    if (port->NextGSCE)
    {
      if (port->Registers.Status.I_CRC_CHK)
      {
    ...
    case AttachWaitDebSink:
        StateMachineAttachWaitDebugSink(port);
        break;
    case AttachedDebSink:
        StateMachineAttachedDebugSink(port);
        break;
    case AttachWaitDebSource:
        StateMachineAttachWaitDebugSource(port);
        break;
    case AttachedDebSource:
        StateMachineAttachedDebugSource(port);
        break;
    case TryDebSource:
        StateMachineTryDebugSource(port);
        break;
    case TryWaitDebSink:
        StateMachineTryWaitDebugSink(port);
        break;
    case UnattachedDebSource:
        StateMachineUnattachedDebugSource(port);
        break;
#endif /* FSC_DTS */
    case IllegalCable:
...
void StateMachineUnattached(Port_t *port)
{
    ...
    if (port->Registers.Status.I_TOGDONE)
    {
        ...
        case 0x5: /* Rp detected on CC1 */
            port->CCPin = CC1;
#ifdef FSC_DTS
            if(port->DTSMode == TRUE)
            {
                SetStateAttachWaitDebugSink(port);
            }
            else
#endif /* FSC_DTS */
            {
     ...
     (defined(FSC_HAVE_SNK) && defined(FSC_HAVE_ACCMODE)))
        case 0x1: /* Rd detected on CC1 */
            ...
            if(port->DTSMode == TRUE)
            {
                SetStateAttachWaitDebugSource(port);
            }
            else
#endif /* FSC_DTS */
            {
...
#endif /* FSC_DTS */
            {
                ...
                if ((port->PortConfig.PortType == USBTypeC_Sink) &&
                    ((port->PortConfig.audioAccSupport) ||
                     (port->PortConfig.poweredAccSupport)))
                {
                    /* TODO - Audio Acc is best choice here. */
                    /* SetStateAttachWaitAccessory(port); */
...
void StateMachineAttachWaitSink(Port_t *port)
{
    ...
    if (port->CCTermCCDebounce == CCTypeRdUSB)
    {
        ...
        /* TODO RICK - Why are we switching pins again? */
        /*
...
void StateMachineAttachWaitSource(Port_t *port)
{
    ...
    if (port->CCTerm == CCTypeOpen && port->VCONNTerm == CCTypeOpen)
    {
        SetStateUnattached(port);
        return;
    }
...
void StateMachineAttachWaitAccessory(Port_t *port)
{
    ...
    if ((port->CCTermCCDebounce == CCTypeRa) && (port->VCONNTerm == CCTypeRa))
    {
        SetStateAudioAccessory(port);
    }
...
void StateMachineAttachWaitDebugSink(Port_t *port)
{
            ...
            || (port->VCONNTerm == CCTypeOpen))
    {
        ...
        if (port->PortConfig.PortType == USBTypeC_DRP)
        {
            SetStateUnattachedDebugSource(port);
        }
        else
#endif /* FSC_HAVE_DRP */
        {
    ...
    else if (port->Registers.Status.VBUSOK)
    {
                ...
                && (port->VCONNTerm < CCTypeUndefined))
        {
            ...
            if (port->PortConfig.PortType == USBTypeC_DRP)
            {
                SetStateTryDebugSource(port);
            }
            else
#endif /* FSC_HAVE_DRP */
            {
                SetStateAttachedDebugSink(port);
            }
        }
    }
}
...
void StateMachineAttachWaitDebugSource(Port_t *port)
{
            ...
            && (port->VCONNTerm < CCTypeUndefined))
    {
        ...
        {
            {
                SetStateAttachedDebugSource(port);
            }
        }
    }
    else if ((port->CCTerm == CCTypeOpen)
...
void StateMachineTryDebugSource(Port_t *port)
{
                    ...
                    && (port->VCONNTerm < CCTypeUndefined)))
    {
        SetStateAttachedDebugSource(port);
            ...
            && (port->CCTerm < CCTypeRdUSB)
            && (port->VCONNTerm < CCTypeRdUSB))
    {
        SetStateTryWaitDebugSink(port);
    }
}
...
void SetStateUnattached(Port_t *port)
{
    ...
    /* Enable the pull-downs on the CC pins */
    port->Registers.Switches.byte[0] = 0x03;
    DeviceWrite(port->I2cAddr, regSwitches0, 1,
        &port->Registers.Switches.byte[0]);
...
void SetStateAttachWaitDebugSink(Port_t *port)
{
    ...
    SetTypeCState(port, AttachWaitDebSink);
    setStateSink(port);
...
void SetStateAttachedDebugSink(Port_t *port)
{
    ...
    SetTypeCState(port, AttachedDebSink);
    setStateSink(port);
...
void SetStateAttachWaitDebugSource(Port_t *port)
{
    ...
    SetTypeCState(port, AttachWaitDebSource);
    setStateSource(port, FALSE);
...
void SetStateAttachedDebugSource(Port_t *port)
{
    port->loopCounter = 0;
    platform_set_vbus_lvl_enable(port->PortID, VBUS_LVL_5V, TRUE, TRUE);
    SetTypeCState(port, AttachedDebSource);
    setStateSource(port, FALSE);
...
{
    SetTypeCState(port, TryDebSource);
    setStateSource(port, FALSE);
...
{
    SetTypeCState(port, TryWaitDebSink);
    setStateSink(port);
...
void SetStateUnattachedDebugSource(Port_t *port)
{
    ...
    SetTypeCState(port, UnattachedDebSource);
    port->CCPin = CCNone;
...
FSC_BOOL isVBUSOverVoltage(Port_t *port, FSC_U16 vbus_mv)
{
    ...
    /* Store the measure register */
    measure = port->Registers.Measure;

    measure.MEAS_VBUS = 1;
    measure.MDAC = (vbus_mv / MDAC_MV_LSB) - 1;
