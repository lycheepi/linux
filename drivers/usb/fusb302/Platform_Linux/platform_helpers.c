...
#ifdef CONFIG_IWG27M
/* IWG27M: Fix for USB_Type-C switching */
#define FUSB_DT_GPIO_CC			        "fairchild,cc"
#endif
...
FSC_S32 fusb_InitializeGPIO(void)
{
    ...
    chip->gpio_Cc = of_get_named_gpio(node, FUSB_DT_GPIO_CC, 0);
    if (!gpio_is_valid(chip->gpio_Cc))
    {
        dev_err(&chip->client->dev, "%s - Error: Could not get named GPIO for CC! Error code: %d\n", __func__, chip->gpio_Cc);
        return chip->gpio_Cc;
    }

    ret = gpio_request(chip->gpio_Cc, FUSB_DT_GPIO_CC);

    ret = gpio_direction_output(chip->gpio_Cc, 1);
    if (ret < 0)
    {
        dev_err(&chip->client->dev, "%s - Error: Could not set GPIO direction to output for CC! Error code: %d\n", __func__, ret);
        return ret;
    }
...
void fusb_GPIO_Set_CC(void)
{
    ...
    if (gpio_is_valid(chip->gpio_Cc))
    {
        ...
        * it may need to be able to sleep. If so, this call must be used.
        */
        if (gpio_cansleep(chip->gpio_Cc))
        {
            gpio_set_value_cansleep(chip->gpio_Cc, (chip->port.CCPin == CC1) ? 0 : 1);
        }
        else
        {
            gpio_set_value(chip->gpio_Cc, (chip->port.CCPin == CC1) ? 0 : 1);
        }
    }
...
static void work_function(struct work_struct *work)
{
    ...
    if (platform_get_device_irq_state(TRUE))
    {
        queue_work(chip->highpri_wq, &chip->sm_worker);
    }
    else
